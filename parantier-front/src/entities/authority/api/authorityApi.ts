...
export const authorityApi = {
  ...
  getByCategory: async (category: string): Promise<Authority[]> => {
    const response = await apiClient.get<Authority[]>(`/authorities/category/${category}`)
    return response.data
  },

  // 역할별 권한 조회
  getByRole: async (role: string): Promise<string[]> => {
    const response = await apiClient.get<string[]>(`/authorities/role/${role}`)
    return response.data
  },

  // 권한 생성
  create: async (data: CreateAuthorityRequest): Promise<Authority> => {
    const response = await apiClient.post<Authority>('/authorities', data)
    return response.data
  },

  // 권한 수정
  update: async (id: number, data: CreateAuthorityRequest): Promise<Authority> => {
    const response = await apiClient.put<Authority>(`/authorities/${id}`, data)
    return response.data
  },
  ...
  updateRoleMapping: async (role: string, data: UpdateRoleMappingRequest): Promise<void> => {
    await apiClient.put(`/authorities/role/${role}/mapping`, data)
  },
}
